...
for f, pp in features.items():
    ...
    if pp["type"] == "cat":
        if pp["random"]: plot_counts(cv_df_filtered, f, ran_df=ran_df, remove_class=pp["remove"], savepath=out_path, verbose=1)
        else: plot_counts(cv_df_filtered, f, remove_class=pp["remove"], savepath=out_path, verbose=1)        

    else: 
        if pp["type"] == "cont":
            if pp["random"]: plot_dists(cv_df_filtered, f, ran_df=ran_df, savepath=out_path, verbose=1)
            else: plot_dists(cv_df_filtered, f, savepath=out_path, verbose=1)
        else: 
            if pp["random"]: plot_integers(cv_df_filtered, f, ran_df=ran_df, savepath=out_path, verbose=1)
