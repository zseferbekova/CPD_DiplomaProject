...
str_classes = ['Surface', 'Core', 'PPI', 'Other']

for c in str_classes:
    print()
    print(f">>>> RESULTS FOR {c.upper()} <<<<")
    print()
    ...
    for f, pp in f_numeric.items():
        ...
        if pp["random"]: 
            res = stat_numeric(cv_df_filtered, f, ran_df=ran_df_c, cont=pp["type"]=="cont")
    ...
    for f, pp in f_categ_mul.items():
        if pp["random"]: 
            res = stat_categ_mul(cv_df_filtered, f, remove=pp["remove"], ran_df=ran_df_c)
    ...
    for f, pp in f_categ.items():
        if pp["random"]: 
            res = stat_categ(cv_df_filtered, f, remove=pp["remove"], ran_df=ran_df_c)
